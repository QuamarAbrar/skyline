...
export const AlertsPanel = memo(function AlertsPanel({ data, onNodeClick }: AlertsPanelProps) {
  // Memoised — only recomputes when data changes, NOT on every render
  const alerts = useMemo<AlertItem[]>(() => {
    const result: AlertItem[] = [];

    data.suspicious_nodes.slice(0, 5).forEach(node => {
