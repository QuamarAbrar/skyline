...
export const GraphView = memo(function GraphView({
  ...
  const { displayNodeIds, displayEdges, isTruncated, hiddenCount, visibleTxCount, totalTxCount } = useMemo(() => {
    if (!data) return { 
    ...
    const scoreMap = new Map(data.suspicious_nodes.map(n => [n.id, n.score.total]));
    ...
    const displayNodeIds = new Set(sorted.slice(0, MAX_RENDER_NODES));
    const totalNodes     = data.nodes.size;
    ...
    return { displayNodeIds, displayEdges, isTruncated, hiddenCount, visibleTxCount, totalTxCount };
  }, [data, timeWindowCutoff]);

  // Ring highlighting
  useEffect(() => {
    if (!cyRef.current || !selectedRingId) return;
  ...
  useEffect(() => {
    ...
    cyRef.current.style(buildStylesheet(nodeColor, edgeColor, outlineColor));
  }, [isDarkMode]);
  ...
  useEffect(() => {
    ...
    try {
      ...
      displayNodeIds.forEach(nodeId => {
        ...
        if (!node) return;
        const suspNode        = data.suspicious_nodes.find(n => n.id === nodeId);
        const suspicionScore  = suspNode?.score.total || 0;
      ...
      const cy = cytoscape({
        ...
        elements,
        style: buildStylesheet(nodeColor, edgeColor, outlineColor),
        layout: {
          name:                       'cose',
          animate:                    false,
  ...
  }, [data, onNodeSelect, displayNodeIds, displayEdges, isDarkMode]);
  ...
  return (
    <div className="relative w-full h-[600px] bg-slate-50 dark:bg-black rounded-xl border border-slate-200 dark:border-neutral-800 overflow-hidden shadow-sm transition-colors duration-300">
      ...
      {loading && (
        <div className="absolute inset-0 flex items-center justify-center bg-white/80 dark:bg-black/80 z-10">
          <div className="flex flex-col items-center">
            ...
            <span className="text-sm text-slate-500 dark:text-neutral-400">Rendering Graph…</span>
          </div>
        </div>
      )}
      ...
      {tooltip && (
        ...
        >
          <div className={`p-3 rounded-lg shadow-2xl border-2 min-w-[200px] ${
            ...
            <div className="flex items-center justify-between mb-2">
              ...
              <span className={`font-bold text-sm ${
                tooltip.riskLevel === 'high'   ? 'text-red-400' :
              ...
              <>
                ...
                <div className="flex flex-wrap gap-1">
                  {tooltip.patterns.slice(0, 3).map((p, idx) => (
                    <span key={idx} className={`px-1.5 py-0.5 rounded text-[9px] ${
                      tooltip.riskLevel === 'high'   ? 'bg-red-900/50 text-red-300' :
      ...
      {/* Zoom controls */}
      <div className="absolute bottom-4 right-4 flex flex-col gap-2 bg-white/90 dark:bg-neutral-900/90 p-2 rounded-lg shadow border border-slate-200 dark:border-neutral-800 z-20">
        <button onClick={handleZoomIn}  className="p-2 hover:bg-slate-100 dark:hover:bg-neutral-800 rounded transition-colors" title="Zoom In">
          <ZoomIn  className="w-4 h-4 text-slate-600 dark:text-neutral-300" />
        </button>
        <button onClick={handleZoomOut} className="p-2 hover:bg-slate-100 dark:hover:bg-neutral-800 rounded transition-colors" title="Zoom Out">
          <ZoomOut className="w-4 h-4 text-slate-600 dark:text-neutral-300" />
        </button>
        <button onClick={handleFit}     className="p-2 hover:bg-slate-100 dark:hover:bg-neutral-800 rounded transition-colors" title="Fit to Screen">
          <Maximize className="w-4 h-4 text-slate-600 dark:text-neutral-300" />
        </button>
        <button
          onClick={() => cyRef.current?.layout({ name: 'cose', animate: true } as any).run()}
          className="p-2 hover:bg-slate-100 dark:hover:bg-neutral-800 rounded transition-colors"
          title="Relayout"
        >
          <RefreshCw className="w-4 h-4 text-slate-600 dark:text-neutral-300" />
        </button>
      </div>
...
// ── Stylesheet factory ────────────────────────────────────────────────────────
function buildStylesheet(nodeColor: string, edgeColor: string, outlineColor: string): cytoscape.Stylesheet[] {
  return [
    {
      selector: 'node',
      style: {
        ...
        'font-size':        '10px',
        color:              nodeColor,
        'background-color': getRiskColor('low'),
        width:  (ele: any) => Math.max(20, Math.min(60, 20 + (ele.data('degree') || 0) * 2)),
        height: (ele: any) => Math.max(20, Math.min(60, 20 + (ele.data('degree') || 0) * 2)),
        'text-outline-width': 2,
        'text-outline-color': outlineColor,
      } as any,
    },
    { selector: 'node[riskLevel = "low"]',    style: { 'background-color': getRiskColor('low')    } },
    { selector: 'node[riskLevel = "medium"]', style: { 'background-color': getRiskColor('medium') } },
    { selector: 'node[riskLevel = "high"]',   style: { 'background-color': getRiskColor('high')   } },
    {
      selector: 'edge',
      style: {
        width:               1,
