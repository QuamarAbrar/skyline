...
class ChunkedGraphEngine {
  ...
  public finalizeMetrics(): void {
    ...
    // Free compact refs now that metrics are finalised
  ...
  private async detectCyclesAsync(): Promise<Set<string>> {
    ...
    for (const [startId, startNode] of this.nodeMap) {
      ...
      while (stack.length > 0) {
        ...
        const neighbours = new Set(node.transactions_out.map(t => t.receiver));
  ...
  private async detectFanPatternsAsync(): Promise<{ fanIn: Set<string>; fanOut: Set<string> }> {
    ...
    for (const [_, node] of this.nodeMap) {
      ...
      const senders = new Set(
        node.transactions_in
          .filter(t => new Date(t.timestamp).getTime() >= windowStart)
          .map(t => t.sender)
      );
      if (senders.size >= THRESHOLDS.FAN_THRESHOLD || node.in_degree >= THRESHOLDS.FAN_THRESHOLD) {
        fanIn.add(node.id);
        // Store this fan-in instance
        this.fanInInstances.push({
          hub: node.id,
          sources: Array.from(senders)
        });
      }

      const receivers = new Set(
        node.transactions_out
          .filter(t => new Date(t.timestamp).getTime() >= windowStart)
          .map(t => t.receiver)
      );
      if (receivers.size >= THRESHOLDS.FAN_THRESHOLD || node.out_degree >= THRESHOLDS.FAN_THRESHOLD) {
        fanOut.add(node.id);
        // Store this fan-out instance
        this.fanOutInstances.push({
          hub: node.id,
          destinations: Array.from(receivers)
        });
      }
    }
